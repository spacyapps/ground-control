...
| **Grok Bot** | yes, grouped | **yes** | every bot under one collapsible group; a decision card waiting on your answer turns that bot and the group red, checked live against Grok Bot's own cache · 2026-08-28. A bot reads as **working** while it owes you a reply or has spoken in the last minute, including one woken by another bot; its name brightens so the state reads without hunting for the dot · 2026-09-19. It runs in xAI's cloud and fires no local hooks, so all of this comes from an undocumented cache — when that cache moved from schemaVersion 3 to 4 the alarm went blind until the reader was taught both shapes, which is the risk of reading a format nobody promised · 2026-09-19 |
