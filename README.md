...
| **OpenAI Codex** | yes | not yet shipped, confirmed reachable | rows read from its own local files — real cwd/name/working-done. Its one real "needs you" moment (an escalated-permission prompt) writes to no file at all while pending, confirmed live — but hooks (`permissionRequest`) are confirmed real and firing, gated by a one-time interactive trust prompt; the alarm needs building, not a workaround · 2026-09-11 |
