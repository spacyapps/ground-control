...
| **Grok Bot** | yes, bots grouped and named | **yes** again, for approvals | 2026-09-23 · 0.58.0 | • one collapsible group, one row per bot, plus the bots another bot wakes<br>• **the alarm works again as of 0.58.0.** `awaitingUserResponse` was null through every test from August on, then started carrying a real block: a local-command approval, with the command written out. The row turns red, says which command wants approving, and clears the instant you answer · 2026-09-23<br>• **a decision card still does not show.** v4 dropped the field that one lived in, so a card — or a question typed in prose — remains invisible. An approval is not<br>• **working is readable**: a bot owes you a reply, or spoke in the last minute. Its name brightens, so the state reads without hunting for the dot<br>• runs in xAI's cloud and fires no local hooks — all of this comes from an undocumented cache, which is why a format change could silence it |
...
| **Claude for Desktop** — Code tab | yes | **yes** | 2026-09-20 | • bundles its own Claude Code and spawns it as a child, so hooks run normally<br>• **a click opens that conversation**, not just the app — it answers a `claude://` link, and the id is read from a file it writes for itself (2026-09-20)<br>• falls back to raising the app whenever that id cannot be found |
| **Claude for Desktop** — cloud session | **no** | no | 2026-09-23 | • a "cloud session" (their term, run by Claude Code Remote) clones your repo into a container Anthropic manages — nothing executes on your Mac, so no hooks run<br>• the desktop app keeps **no local record** of it either: its session store held only local sessions, and nothing on disk named the cloud one<br>• unlike Grok Bot, there is no cache to read. Seeing it would mean asking claude.ai over the network, and this app has no network code |
