...
| **Codex's hook payload is Claude's dialect, snake_case on stdin** | captured in full with `Scripts/probe-codex-hooks.sh` — all 12 events registered, one real interactive turn: `session_id`, `cwd`, `transcript_path`, `hook_event_name`, `tool_name`, `last_assistant_message`, `permission_mode`. Not the camelCase the app-server wire schema uses; `cc-notify` needs no new dialect | 2026-09-11 |
| **`PermissionRequest` fires the instant Codex blocks** | same capture: the event landed at 19:30:45 carrying `tool_input.description` as a written-out question, `PostToolUse` at 19:32:05 — 80 seconds in which nothing was written to any file. The alarm the file-based producer cannot see is a push away | 2026-09-11 |
| **`SessionEnd` fires on a clean quit**, with a `reason` | same capture — so a Codex row can be retired per *thread*, not by `CodexWatcher.showEndedFor`'s by-folder 30-minute guess | 2026-09-11 |
| **Codex sub-agents DO have their own rollout files** — an earlier claim here was wrong | the `agent_thread_id` in the parent's inline `SubAgentActivity` is byte-for-byte the `agent_id` on the hook and the id in a real 47KB rollout filename. The earlier pass checked `session_index.jsonl`, where children have 0 entries against the parent's 1, and read "unindexed" as "does not exist" | 2026-09-11 |
| **Codex's `SubagentStart` fires, and fires *before* the work** | captured live with two real sub-agents: parent `session_id` + child `agent_id` in one payload, `agent_transcript_path` on stop. Better than Claude (stop only) and better than Grok (whose `subagentStart` was asserted from docs, then measured as never firing) | 2026-09-11 |
| `transcript_path` flips meaning between the two subagent events | `SubagentStart` puts the *child's* rollout there; `SubagentStop` puts the *parent's*, moving the child to `agent_transcript_path`. Key on `session_id` + `agent_id` instead | 2026-09-11 |
